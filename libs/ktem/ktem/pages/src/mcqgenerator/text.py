...
// 每5分钟检查一次鼠标位置
setInterval(checkMousePosition, 300000);
...
def create_group_of_users(file_exp, file_con, start_range_1, end_range_1, start_range_2, end_range_2):
...
with gr.Blocks(head=custom_js) as demo:
    file_path = os.path.join("libs/ktem/ktem/pages/src/study_time", f"total_time_{user_id}.json")
    ...
    with gr.Row():
        E_range1 = gr.Number(label="实验组学生学号开始区间")
        E_range2 = gr.Number(label="实验组学生学号结束区间")
    with gr.Row():
        C_range1 = gr.Number(label="对照组学生学号开始区间")
        C_range2 = gr.Number(label="对照组学生学号结束区间")
    with gr.Row():
        exp_file = gr.File(label="上传实验组学生信息XLS文件")
        con_file = gr.File(label="上传对照组学生信息XLS文件")
    btn_group = gr.Button("Create group")

    btn_group.click(
        create_group_of_users,
        inputs=[exp_file, con_file,
                E_range1, E_range2,
                C_range1, C_range2],
    ...
    def handle_mouse_coordinates():
        current_time = time.time()
        if time.time() - state_data["last_activity_time"] > 400:
            # 非活动状态，初始化数据
            ...
            with open(os.path.join("libs/ktem/ktem/pages/src/study_time", f"total_time_{user_id}.json"), 'w',
                ...
                json.dump({"total_active_time":state_data["last_activity_time"]}, S_file, ensure_ascii=False, indent=4)


    # 文本框用于显示鼠标坐标
    mouse_x = gr.Textbox(label="Mouse Coordinates", elem_id="mouse-coordinates")

    # 隐藏按钮用于触发后端更新
    update_button = gr.Button("Update", visible=False, elem_id="update-coordinates")

    # 当按钮被点击时，调用后端处理函数
    update_button.click(fn=handle_mouse_coordinates)

# 启动应用
demo.launch(inbrowser=True, server_name="172.18.232.73", share=False)
