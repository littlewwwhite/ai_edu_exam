...
class GradioMCQPage(BasePage):
    ...
    def on_building_ui(self):
        with gr.Blocks(head=custom_js) as self.mcq_interface:
            ...
            with gr.Tab("题目生成"):
                with gr.Row():
                    with gr.Column(scale=1):
                        ...
                        self.study_time_display = gr.Markdown("")
            ...
            ).then(fn=self.init,inputs=[self._app.user_id])
    ...
    def handle_mouse_coordinates(self, user_id):
        if self.flag:
            print("触发成功！")
            current_time = time.time()
            if time.time() - self.state_data["last_activity_time"] > 3:
                # 非活动状态，初始化数据
                print("# 非活动状态，初始化数据")
                self.state_data["is_active"]=False
                self.state_data["last_activity_time"]=current_time
            else:
                # 已开始活动或者还在活动
                if self.state_data["is_active"]:
                    print("活动")
                    self.state_data["total_active_time"] += current_time - self.state_data["last_activity_time"]
                    self.state_data["last_activity_time"] = current_time
                else:
                    print("已开始活动")
                    self.state_data["last_activity_time"] = current_time
                ...
                with open(os.path.join("libs/ktem/ktem/pages/src/study_time", f"total_time_{user_id}.json"), 'w',
                    ...
                    json.dump({"total_active_time":self.state_data["last_activity_time"]}, S_file, ensure_ascii=False, indent=4)
            return f"您的学习总时长为：{self.state_data['last_activity_time']}"
        return "您的学习总时长为：0.0"